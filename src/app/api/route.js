...
export async function GET(request) {
  ...
  // Linisin ang input ID upang makuha ang tamang short ID
  var cleanId = id;
  if (id.indexOf('/s/') !== -1) {
    cleanId = id.split('/s/')[1].split('?')[0].split('&')[0];
  } else if (id.indexOf('surl=') !== -1) {
    cleanId = id.split('surl=')[1].split('&')[0];
  }

  var finalShareUrl = "https://www.terabox.com/s/" + cleanId;

  // ------------------------------------------------------------
  // SOURCE A: TWO-STEP PUBLIC WORKER RESOLVER (MATATAG AT MABILIS)
  // ------------------------------------------------------------
  try {
    var infoRes = await fetch("https://terabox.hnn.workers.dev/api/get-info?shorturl=" + cleanId + "&pwd=", {
      method: 'GET',
    ...
    if (infoRes.ok) {
      ...
      var fsId = infoData.list && infoData.list[0] && infoData.list[0].fs_id;

      if (fsId) {
        var dlRes = await fetch("https://terabox.hnn.workers.dev/api/get-download", {
          method: 'POST',
          headers: {
        ...
        if (dlRes.ok) {
          ...
          var rawDirect = dlData.downloadLink || dlData.download_link || dlData.direct_link || dlData.download;

          if (rawDirect) {
            // Gumamit ng standard double-quotes concatenation para iwas copy-paste error sa selpon
            var proxiedLink = "https://teradl.shraj.workers.dev/?url=" + encodeURIComponent(rawDirect);
  ...
  for (const apiUrl of apiSources) {
    try {
      ...
      if (response.ok) {
        ...
        var fileList = data.files || data.list;
        var file = fileList && fileList[0];
        
        var rawDirectLink = file ? (file.download_link || file.direct_link || file.dlink) : null;
        var fileName = file ? (file.filename || file.file_name) : "TeraBox_Video.mp4";

        if (rawDirectLink) {
          var proxiedDownloadLink = "https://teradl.shraj.workers.dev/?url=" + encodeURIComponent(rawDirectLink);
          return NextResponse.json({
            ok: true,
  ...
  // ------------------------------------------------------------
  try {
    var terabridgeUrl = "https://terabridge.vercel.app/api/download?surl=" + cleanId;
    return NextResponse.json({
      ok: true,
